#!/bin/bash
# From: https://github.com/vkulyk-mindk/bareos-zabbix
set -ex
...
case $bareosDbSgdb in
  P) sql="env PGPASSWORD=$bareosDbPass  /usr/bin/psql -h$bareosDbAddr -p$bareosDbPort -U$bareosDbUser -d$bareosDbName -tA -c" ;;
...
bareosJobType=$($sql "select Type from Job where JobId=$bareosJobId;")

if [ "$bareosJobType" != "B" ] ; then exit 9 ; fi

# Get Job level from database and classify it as Full, Differential, or Incremental
bareosJobLevel=$($sql "select Level from Job where JobId=$bareosJobId;")
...
bareosJobStatus=$($sql "select JobStatus from Job where JobId=$bareosJobId;")
...
bareosClientName=$($sql "select Client.Name from Client,Job where Job.ClientId=Client.ClientId and Job.JobId=$bareosJobId;")
...
if [[ -z $bareosJobName ]] ; then exit 17 ; fi
